...
public class Task4_Exam {
    ...
    public static TruckType getTypeByWeight(int weight) {
        return Stream.of(TruckType.values())
                .filter(car -> weight <= car.maxLoad)
                .min(Comparator.comparingInt(car -> car.maxLoad))
    ...
    public static Map<TruckType, List<Truck>> groupTrucksByType(List<Truck> trucks) {
        return trucks.stream()
                .collect(Collectors.groupingBy(car -> {
                    try {
                        return getTypeByWeight(car.maxWeightKg);
                    } catch (WeightTooHighException ignore) {
                        return Arrays.stream(TruckType.values()).max(Comparator.comparingInt(e -> e.maxLoad)).get();
                    }
                }));
    }
    ...
    public static Map<TruckType, Long> countTrucksByType(List<Truck> trucks) {
        return trucks.stream()
                .collect(Collectors.groupingBy(car -> getTypeByWeight(car.maxWeightKg), Collectors.counting()));
    }
